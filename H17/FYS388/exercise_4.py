...
def exercise_4_2():
    ...
    soma_ecc = return_soma()
    stim_ecc = insert_current_clamp(soma_ecc(0.5))
    _, v_ecc = run_simulation(soma_ecc(0.5))

    fig = plt.figure(2)
    ax1 = fig.add_subplot(111, xlabel="Time [ms]", ylabel="Voltage [mV]")
    ax1.plot(t, exp_somav, 'gray', label='"Experiment"')
    ax1.plot(t, v_rall, 'g', label='Rall')
    ax1.plot(t, v_ecc, 'b', label='Ecc')
    plt.legend(loc=4, frameon=False)

    plt.savefig('exercise_4_2_.png')
    plt.show()
...
def exercise_4_3():
    ...
    ax1 = fig.add_subplot(111, xlabel="Time [ms]", ylabel="Voltage [mV]")
    for i in x:
        stim = insert_current_clamp(soma_rall(i))
        t, v_rall = run_simulation(soma_rall(i))
        ax1.plot(t, v_rall)
    plt.show()
