...
def set_parameters():
	...
	## stimulus parameters
	p['I_amp'] = 50.           ## input current amplitude (uA/cm2)
...
def f_I():
	...
	for I_value in I_values:
		...
		for V_value in Vm:
			...
			if V_value >= -20 and blabla:
				event += 1
				blabla = False
			elif V_value < -20 and blabla == False:
				blabla = True
		f.append(0.5*event/(p["T"]))
	return f, I_values
